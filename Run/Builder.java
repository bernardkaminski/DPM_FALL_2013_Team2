...
public class Builder {
		...
		/**
		 * This class is responsible for executing the builder function of the robot
        ...
        static void main(Odometer odo, TwoWheeledRobot robo,Navigation nav,Search search,lightlocalization loc,Map map,int startingCorner ) 
        {
	        ...
	        {       
                ...
                if(i==xCords.size()-1)
                { 
                	//Point to travel too is the green zone, travel there and break loop
		    ...
		    if(hasBlock)
		    {
	            ...
	            {
	            	//This is a hard code to avoid hitting the blue block out of its position when the robot leaves the green zone
	            	//Only applies if the green zone is in the left half of the field, because of we always go to the bottom left corner of green zone
		     ...
		     //Generate new path to next "green zone" (point chosen to ensure that robot covers previously uncovered ground), starting at current green zone
		    	 ...
		    	 if(yCords.get(yCords.size()-1)>yCords.get(0))
	             {
	              ...
	              else
	              {
		    	   ...
		    	   //Turn back after scan
			        ...
			        {
		                hasBlock=true;
		                //Go back to green zone to stack using exact same path, because we already know that it is clear
        	...
        	//Robot has stacked 2 blue blocks in green zone, assuming it found a second block while navgiating to the mock green zone
	}